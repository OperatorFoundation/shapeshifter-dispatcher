...
FILENAME=testUDPMeekOutput.txt
...
~/go/bin/shapeshifter-dispatcher -transparent -udp -server -state state -orport 127.0.0.1:3333 -transports meekserver -bindaddr meekserver-127.0.0.1:2222 -optionsFile ../../ConfigFiles/meek.json -logLevel DEBUG -enableLogging &

sleep 1

# Run the transport client
~/go/bin/shapeshifter-dispatcher -transparent -udp -client -state state -target 127.0.0.1:2222 -transports meeklite -proxylistenaddr 127.0.0.1:1443 -optionsFile ../../ConfigFiles/meek.json -logLevel DEBUG -enableLogging &
